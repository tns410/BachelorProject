...
stop_times = sorted(routes.values(), key=lambda item: len(item), reverse=True)[:10]
stop_times = [stop_time for route in stop_times for trip in route.values() for stop_time in trip]
