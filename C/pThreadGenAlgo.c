...
float get_min_efficiency(struct schedule schedules[]) {
    float min_efficiency = schedules[0].efficiency;
    for (int schedule_idx = 0; schedule_idx < MAX_NR_NETWORKS; ++schedule_idx) {
...
int main()
{
    ...
    int epoch = 1;
    while (true) {
        pthread_t threads[MAX_NR_NETWORKS];
        for (int thread_idx = 0; thread_idx < MAX_NR_NETWORKS; ++thread_idx) {
            int rc = pthread_create(&threads[thread_idx], NULL, thread_func, (void *)&schedules[thread_idx]);
            if (rc) {
