...
int main(int argc, char **argv)
{
    ...
    if (rank == 0) {
        ...
        int epoch = 1;
        for (int i = 0; i < 5; ++i) {
            struct schedule * schedules = calloc(MAX_NR_CALCS, sizeof(struct schedule));
            ...
            for (int schedule_idx = 0; schedule_idx < MAX_NR_CALCS; ++schedule_idx) {
                struct schedule * schedule = &schedules[schedule_idx];
                update_offsets(schedule->offsets, first->offsets, routes, schedule_idx);
            }

            for (int loop = 0; loop <= MAX_NR_CALCS / num_procs; ++loop) {
                int calc_idx;
                for (int proc = 1; proc < num_procs; ++proc) {
                    calc_idx = loop * num_procs + proc;
                    if (calc_idx < MAX_NR_CALCS) {
                        int * offsets = schedules[calc_idx].offsets;
                        int size = 0;
                        for (int idx = 0; idx < MAX_NR_ROUTES; idx++)
                            size += sprintf(&offset_buf[size], "%d|", offsets[idx]);
                        MPI_Send(offset_buf, sizeof(offset_buf), MPI_CHAR, proc, 0, MPI_COMM_WORLD);
                    }
                }

                calc_idx = loop * num_procs;
                if (calc_idx < MAX_NR_CALCS) {
                ...
                for (int proc = 1; proc < num_procs; ++proc) {
                    calc_idx = loop * num_procs + proc;
                    if (calc_idx < MAX_NR_CALCS) {
                        MPI_Recv(result_buf, sizeof(result_buf), MPI_CHAR, proc, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                        struct schedule *schedule = &schedules[calc_idx];
            ...
            for (int calc_idx = 0; calc_idx < MAX_NR_CALCS; calc_idx++) {
                struct schedule * schedule = &schedules[calc_idx];
